...
"""
Module to simulate the deformation of a scanned page. Inspired form the blog
page https://mzucker.github.io/2016/08/15/page-dewarping.html
"""
...
class BookDeform:
    """
    This class generate a surface deformed by a BSpline and compute the
    coordinates transfer for a texture to be wrapped on it.
    """

    def __init__(self, width: int, height: int, alpha: float, beta: float):
        """
        Generates a flat 2D rectangular page as a meshgrid.

        Args:
            width: int
                Width of the page (in pixel).
            height: int
                Height of the page (in pixel).
            alpha: float
                Desired slope (derivative) at x = 0.
            beta: float
                Desired slope at x = 1.
        """
        self.xx, self.yy, self.zz = self.generate_flat_page(width=width,
                                                            height=height)
        self.xx_deformed, \
            self.yy_deformed, \
            self.zz_deformed, \
            self.xx_scaled = (
                self.deform_page(self.xx, self.yy, alpha, beta)
            )
    ...
    def get_surface(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: Meshgrids (X, Y, Z)
                representing a flat page in 3D space, with Z set to the
                deformation field. All normalized.
        """
        # the deform is proportionnal to the width, i.e. the "x" component
        return (
            self.xx_deformed / np.max(self.xx_deformed),
    ...
    def generate_flat_page(
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: Meshgrids (X, Y, Z)
                representing a flat page in 3D space,
    ...
    def generate_bspline(
        ...
        """
        Generate a custom cubic B-spline curve constrained by derivative
        values at its endpoints.

        The spline is defined as a cubic polynomial
        f(x) = ax^3 + bx^2 + cx + d, where:
          - f(0) = 0 and f(1) = 0 (start and end at zero)
          - f'(0) = alpha (controls initial slope)
          - f'(1) = beta  (controls final slope)

        These constraints create a flexible spline that can
        simulate local bending.

        Args:
            num_alpha (float): Desired slope (derivative) at x = 0.
            num_beta (float): Desired slope at x = 1.
            xvals (np.ndarray): 1D array of x-values at which to evaluate
                the spline.

        Returns:
            np.ndarray: 1D array of y-values corresponding
                to the spline evaluated at xvals.
            np.ndarray: 1D array of the positive integral of the
                spline along the way
        """
        ...
        for x in range(nb_point):
            if x == 0:
                ...
                integral[x] = integral[x - 1] + self.curve_length_cubic(
                    coeffs, (x - 1) / np.max(xvals), x / np.max(xvals)
                ) * np.max(xvals)

        return deform_vals, integral

    def curve_length_cubic(self,
                           coeffs: Union[list[float], np.ndarray],
                           a: float,
                           b: float) -> float:
        """
        Compute the arc length of a cubic polynomial curve y = f(x)
        between x=a and x=b.

        Args:
            coeffs (Iterable): Coefficients [d, c, b, a] for the cubic
    ...
    def deform_page(
        ...
        """
        Apply a 2-parameters constraint spline to create a 3D warp (to a latter
        apply to a flat page to simulate bending).
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]: Deformed 3D
                surface coordinates (X, Y, Z).
        """
        zz, integral = self.generate_bspline(
            alpha, beta, xx
        )  # Spline wave deformation in Z direction
    ...
    def project_image_on_surface(self,
                                 image_to_distord: np.ndarray) -> np.ndarray:
        """
        ...
        Returns:
            np.ndarray: Deformed image to be mapped on the 3D surface
                coordinates (X, Y, Z).
        """
