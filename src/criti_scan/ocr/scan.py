...
from criti_scan.ocr.detect_zones import ZoneDetecter
from criti_scan.ocr.ocr import OCRText
...
@click.option('--language',
              '-l',
              required=True,
              help='OCR language code (e.g., "eng", "grc").')
@click.option('--tessdata-dir', '-t',
              required=True,
              type=click.Path(exists=True, file_okay=False),
              help='Path to the directory containing the tessdata.')
@click.option('--rm-temp-dir/--keep-temp-dir',
              default=True,
              help='Whether to remove intermediate'
              'files after OCR (default: remove).')
